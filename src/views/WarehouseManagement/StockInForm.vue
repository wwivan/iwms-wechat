    ...
    <div class="content">
      ...
      <div class="row">
        <date-select-cell class="text-left"
                          ...
                          :title="'入库时间'"
                          :placeholder="'请输入入库时间'"
                          :value.sync="params.purchaseDateVO"></date-select-cell>
      </div>
      <div class="row d-flex ai-center"
        ...
        <van-button v-if="selectedStockType.id == '1'|| selectedStockType.id == '3' || selectedStockType.id == '4' || selectedStockType.id == '2'"
                    size="small"
                    @click="selectSupplier(selectedStockType,selectedWarehouse,params.deliveryNumber,params.purchaseNo)"
                    style="border-radius:0.05rem;background: linear-gradient(135deg, #FF9779, #F6617B);color:white;">选择</van-button>
      </div>
      <div class="row">
        <span style="margin-left:0"
      ...
      <div class="row"
           v-if="selectedStockType.id == '6'">
        <span style="margin-left:0"
              class="title">提交人</span>
        <input type="text"
               style="text-align:right;border:0 solid rgba(0,0,0,0.25);font-size:0.13rem"
               :placeholder="'请输入提交人'"
